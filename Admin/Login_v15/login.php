...
if(isset($_POST['submit'])){
    ...
    if(empty($_POST['email'])){
        ...
        if(empty($_POST['password'])){
            ...
            $Pseudo = htmlentities($_POST['email'], ENT_QUOTES, "UTF-8"); 
            $MotDePasse = htmlentities($_POST['password'], ENT_QUOTES, "UTF-8");
            //on se connecte à la base de données:
            $mysqli = mysqli_connect("domaine.tld", "root", "", "zikearch");
            ...
            if(!$mysqli){
                ...
                $Requete = mysqli_query($mysqli,"SELECT * FROM users WHERE email = '".$email."' AND password = '".$pass."'");
